...
class Config(object):
    ...
    def _make_absolute(self, path, relative_to):
        return (
            path if path.startswith('http') else os.path.abspath(
    ...
    def script_urls(self):
        ...
        return [
            "__jasmine__/{0}".format(core_js)
            for core_js in core_js_files
        ] + [
            self._prefix_src_underscored(src_file)
            for src_file in self.src_files()
        ] + [
            "__spec__/{0}".format(helper)
            for helper in self.helpers()
        ] + [
            "__spec__/{0}".format(spec_file)
